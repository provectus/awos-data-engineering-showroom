...
with aggregated_demand as (
    select
        ...
        -- Average metrics across all games for this stadium/station/interval combination
    ...
    group by stadium_name, station_id, interval_offset, hour_offset
)
...
select
    ...
    case
        when ad.avg_net_flow < -3 then round(abs(ad.avg_net_flow))  -- Add bikes (positive number)
        when ad.avg_net_flow > 3 then -1 * round(ad.avg_net_flow)   -- Remove bikes (negative number)
        else 0
    end as bikes_to_add_or_remove,

    -- Text recommendation
    case
        when ad.avg_net_flow < -3 then 'Add ' || round(abs(ad.avg_net_flow)) || ' bikes'
        when ad.avg_net_flow > 3 then 'Remove ' || round(ad.avg_net_flow) || ' bikes'
        else 'No action needed'
    end as rebalancing_recommendation,

    -- Percentage capacity adjustment (assuming ~20 bikes per station capacity)
    case
        when ad.avg_net_flow < -3 then round((abs(ad.avg_net_flow) / 20.0) * 100, 1)
        when ad.avg_net_flow > 3 then -1 * round((ad.avg_net_flow / 20.0) * 100, 1)
        else 0
...
order by ad.stadium_name, abs(ad.avg_net_flow) desc, ad.station_id, ad.interval_offset
